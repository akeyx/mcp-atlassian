...
| `body` | `string` | Yes | Comment text in Markdown format. On Jira Cloud, embed an existing issue attachment with `![alt](filename.png)`. |
...
<Tip>
Supports Markdown formatting. On Jira Cloud, embed an existing issue attachment with `![alt](filename.png)`, and mention users with `[~accountid:ACCOUNT_ID]` or `@[Display Name](accountid:ACCOUNT_ID)`. Use `visibility` parameter for restricted comments (e.g., service desk internal notes). The generated schema lists `body`, but callers may use `comment` as an equivalent input alias for the same comment text.
</Tip>
...
| `body` | `string` | Yes | Updated comment text in Markdown format. On Jira Cloud, existing attachment images are preserved; embed another with `![alt](filename.png)`. |
| `visibility` | `string` | No | (Optional) Comment visibility as JSON string (e.g. '`{"type":"group","value":"jira-users"}`') |
